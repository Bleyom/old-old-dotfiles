...
chosen="$(echo -e "$options" | $rofi_command -dmenu -selected-row 2 -lines 5 -width 16)"
case $chosen in
    $shot_now)
        ...
        ;;
    $select_shot)
        scrot 'snap_%y_%m_%d_%H_%M.png' -s -q 100 -e  'mv $f ~/Documents/Snapshots/ ; notify-send Screenshot -i ~/Documents/Snapshots/$f -u normal "Snapshot Taken"'
        dunstify
                ;;
        $shot_2)
